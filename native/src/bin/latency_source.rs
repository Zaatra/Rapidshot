...
unsafe fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let class = WNDCLASSW {
        ...
        lpszClassName: w!("RapidshotLatency"),
        ..Default::default()
    };
    ...
    let hwnd = CreateWindowExW(
        WS_EX_TOPMOST,
        w!("RapidshotLatency"),
        w!("Rapidshot latency source - Esc stops"),
        WS_POPUP,
        0,
        0,
        width as i32,
    ...
    let vs = compile(
        PCSTR(c"VS".as_ptr().cast()),
        PCSTR(c"vs_5_0".as_ptr().cast()),
    )?;
    let ps = compile(
        PCSTR(c"PS".as_ptr().cast()),
    ...
    loop {
        ...
        if stopped.load(Ordering::SeqCst) || start.elapsed().as_secs() > 3600 {
            break;
        }
        ...
        if status != windows::core::HRESULT(0) {
            return Err(format!("Present not visible/successful: {status:?}").into());
        }
        writeln!(
            log,
            "{{\"event\":\"present\",\"id\":{id},\"qpc_before\":{before},\"qpc_after\":{after}}}"
        )?;
        if id == 1 {
            log.sync_data()?;
