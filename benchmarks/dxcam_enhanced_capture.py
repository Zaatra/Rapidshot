...
parser = argparse.ArgumentParser(description='DXCam Enhanced Capture Benchmark')
...
title = "[DXCam Enhanced] Capture Benchmark"
...
# Create camera with specified options
camera = dxcam.create(output_idx=0, output_color=args.color, nvidia_gpu=args.gpu)

# Start performance timing
start_time = time.perf_counter()

# Start capture
camera.start(region=region, target_fps=args.target_fps, video_mode=args.video_mode)
...
for i in range(args.frames):
    ...
    image = camera.get_latest_frame(as_numpy=args.as_numpy)
