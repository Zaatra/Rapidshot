...
from ._paths import worker_command, worker_env

MODULE = "rapidshot._bench.ai_ingestion"
OUT = 640
...
def _child_options():
    return {"env": worker_env({**os.environ, "PYTHONIOENCODING": "utf-8"}),
...
class MotionSource:
    ...
    def start(self):
        ...
        self.proc = subprocess.Popen(
            worker_command("rapidshot._bench.motion_source",
...
def spawn(path: str, seconds: float, warmup: int, verify: bool, *, logs=None,
    ...
    cmd = worker_command(MODULE, "--call-duration", "--worker", path,
