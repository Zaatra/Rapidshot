# Copied from examples/gpu_tensor_to_cupy.py so that `rapidshot benchmark` can
# run from a wheel, where examples/ does not exist. The only difference is the
# sys.path line the example needs to run from a checkout, which here would put
# rapidshot/ itself on the path. tests/test_bench_package.py pins the rest.
